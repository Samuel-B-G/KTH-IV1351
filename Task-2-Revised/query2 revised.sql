CREATE OR REPLACE VIEW course_instance_allocated_hours AS (
	SELECT
		...
		ROUND(SUM(CASE WHEN ta.activity_name = 'Lecture'
			THEN aa.allocated_hours * ta.factor ELSE 0 END)::NUMERIC, 2) AS "Lecture Hours",
		ROUND(SUM(CASE WHEN ta.activity_name = 'Lab'
			THEN aa.allocated_hours * ta.factor ELSE 0 END)::NUMERIC, 2) AS "Lab Hours",
		ROUND(SUM(CASE WHEN ta.activity_name = 'Tutorial'
			THEN aa.allocated_hours * ta.factor ELSE 0 END)::NUMERIC, 2) AS "Tutorial Hours",
		ROUND(SUM(CASE WHEN ta.activity_name = 'Seminar'
			THEN aa.allocated_hours * ta.factor ELSE 0 END)::NUMERIC, 2) AS "Seminar Hours",
			
		ROUND(SUM(CASE WHEN ta.activity_name = 'Exam'
			THEN aa.allocated_hours * ta.factor ELSE 0 END)::NUMERIC, 2) AS "Exam Hours",
		ROUND(SUM(CASE WHEN ta.activity_name = 'Admin'
			THEN aa.allocated_hours * ta.factor ELSE 0 END)::NUMERIC, 2) AS "Admin Hours",
			
		ROUND(SUM(aa.allocated_hours * ta.factor)::NUMERIC, 2) AS "Total Hours"
